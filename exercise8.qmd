...
Finally, we will use the bootstrapping method to calculate the upper and lower 95% confidence intervals of our model. After running the bootstrap using the do() function from {mosaic} we will calculate the confidence intervals by using the quantile method and theory based method. The CI's from both these methods indicate that the slope coefficient is not different from 0, as they both intersect with 0, with the lower interval being negative and the upper interval being positive.
